...
// import 'package:mobileapp2/services/user_service.dart';
...
class _UserRiwayatPageState extends State<UserRiwayatPage>
  ...
  int get _totalSpend => _riwayat
      .where((r) => (r['status'] ?? '').toString().toLowerCase() == 'completed')
      .fold(0, (s, r) => s + (int.tryParse(r['total']?.toString() ?? '0') ?? 0));

  // ── Detail transaksi ─────────────────────────────────────
  void _showDetail(dynamic trx) {
    final status = (trx['status'] ?? '').toString();
    final sc     = _statusColor(status);
    final items  = trx['items'] as List? ?? trx['detail'] as List? ?? [];
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (items.isEmpty)
                        ...
                        ...items.map((item) {
                          final img = item['barang']?['image'] ??
                              item['image'] ?? '';
                          final imgUrl = img.toString().isEmpty ? ''
                              : img.toString().startsWith('http') ? img.toString()
                              : 'https://learn.smktelkom-mlg.sch.id/toko/$img';
                          final nama  = item['barang']?['nama_barang'] ??
                              item['nama_barang'] ?? item['nama'] ?? '-';
                          final harga = item['harga'] ??
                              item['barang']?['harga'] ?? 0;
                          final qty   = item['qty'] ?? item['jumlah'] ?? 1;
                          final sub   = (int.tryParse(harga.toString()) ?? 0) *
                            ...
                            child: Row(children: [
                              ...
                              Expanded(child: Column(
                                ...
                                children: [
                                  ...
                                  Text('${qty}x ${_formatRp(int.tryParse(harga.toString()) ?? 0)}',
                                      style: TextStyle(fontSize: 11,
                                          color: Colors.white.withOpacity(0.4),
                                          fontFamily: 'Poppins')),
                                ],
  ...
  Widget _buildAppBar() => SliverAppBar(
    ...
    expandedHeight: 80, collapsedHeight: 80,
      ...
      title: FadeTransition(
        ...
        child: SlideTransition(
          ...
          child: Row(children: [
            ...
            GestureDetector(
              onTap: () { HapticFeedback.lightImpact(); _loadRiwayat(); },
              child: Container(
                width: 38, height: 38,
                decoration: BoxDecoration(
                    color: Colors.white.withOpacity(0.07),
  ...
  Widget _buildSummaryRow() => SliverToBoxAdapter(
    child: FadeTransition(
      opacity: _headerFade,
      child: SizedBox(height: 96, child: ListView(
        scrollDirection: Axis.horizontal,
        padding: const EdgeInsets.fromLTRB(20, 0, 20, 8),
        children: [
          _statPill('Total Transaksi', '${_riwayat.length}',
              Icons.receipt_outlined, _primary),
          _statPill('Selesai',
              '${_riwayat.where((r) => (r['status'] ?? '').toString().toLowerCase() == 'completed').length}',
              Icons.check_circle_outline, _green),
          _statPill('Pending',
              '${_riwayat.where((r) => (r['status'] ?? '').toString().toLowerCase() == 'pending').length}',
              Icons.pending_actions_rounded, _amber),
          _statPill('Total Spend', _formatRp(_totalSpend),
  ...
  Widget _buildList() {
      ...
      return SliverFillRemaining(hasScrollBody: false,
        child: Center(child: Column(
          ...
          children: [
            ...
            Text('Transaksimu akan muncul di sini',
                style: TextStyle(fontSize: 12,
                    color: Colors.white.withOpacity(0.4),
                    fontFamily: 'Poppins')),
          ],
    ...
    return SliverPadding(
      ...
      sliver: SliverList.builder(
        ...
        itemBuilder: (_, i) => _buildTrxCard(list[i], i),
      ),
    );
  }
  ...
  Widget _buildTrxCard(dynamic trx, int index) {
    ...
    final sc      = _statusColor(status);
    final items   = trx['items'] as List? ?? trx['detail'] as List? ?? [];
    final total   = int.tryParse(trx['total']?.toString() ?? '0') ?? 0;
    final isPend  = status.toLowerCase() == 'pending';
    ...
    return TweenAnimationBuilder<double>(
      ...
      child: GestureDetector(
        onTap: () { HapticFeedback.lightImpact(); _showDetail(trx); },
        child: Container(
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (items.isEmpty) ...[
                    Text('${trx['jumlah_item'] ?? items.length} item',
                        style: TextStyle(fontSize: 12,
                            color: Colors.white.withOpacity(0.4),
                            fontFamily: 'Poppins')),
                  ],
                      ...
                      child: Row(children: [
                        ...
                        Text('Menunggu konfirmasi penjual',
                            style: const TextStyle(fontSize: 11,
                                color: _amber, fontFamily: 'Poppins',
                                fontWeight: FontWeight.w600)),
                      ]),
                    ),
                  ],
